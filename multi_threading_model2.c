...
pthread_mutex_t mutex_for_count;
...
void* merge_array(void* thread)
{
	...
	//pthread_mutex_lock(&mutex_for_count);
	long count = sort_using_threads->count++;
	//pthread_mutex_unlock(&mutex_for_count);
...
int main()
{
	...
	if(filetobeopened)
	{
		...
		long i,n,p,no_of_threads,size_of_array,reserved,k = 0,base = 1,error,phase_count = 0;
		...
		//a mutex for stopping over bound
		pthread_mutex_init(&mutex_for_count,NULL);
		...
		pthread_mutex_destroy(&mutex_for_count);
